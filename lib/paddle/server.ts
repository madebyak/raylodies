...
async function paddleRequest<T>(
  ...
  if (!apiKey) {
    ...
    return {
      error:
        "Paddle API key not configured. Add PADDLE_API_KEY to your environment variables.",
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        error: error.error?.detail || "Failed to communicate with Paddle",
      };
    }
 ...
 */
export async function syncProductWithPaddle(options: {
  ...
  if (paddleProductId) {
    ...
    const productResult = await updatePaddleProduct(
      paddleProductId,
      name,
      description,
    );
    if (productResult.error) {
      return {
        error: `Failed to update Paddle product: ${productResult.error}`,
      };
    }
  } else {
    // Create new product
    ...
    if (productResult.error) {
      return {
        error: `Failed to create Paddle product: ${productResult.error}`,
      };
    }
  ...
  // Create new price
  const priceResult = await createPaddlePrice(
    paddleProductId!,
    price,
    currency,
  );
