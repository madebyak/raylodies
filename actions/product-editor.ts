...
export async function upsertProduct(formData: FormData) {
  ...
  if (id && id !== "new") {
    ...
    slug =
      existing?.slug || (await generateUniqueProductSlug(supabase, title, id));
    existingDbPrice =
      typeof existing?.price === "number" ? existing.price : null;
  }
  if (!isEdit) {
  ...
  const paddleResult = is_free
    ...
    : await syncProductWithPaddle({
        ...
        price,
        updatePrice:
          existingDbPrice === null ? true : existingDbPrice !== price,
      });
  ...
  const productData = {
    ...
    paddle_product_id: is_free
      ? null
      : paddleResult.paddleProductId || existingPaddleProductId || null,
    paddle_price_id: is_free
      ? null
      : paddleResult.paddlePriceId || existingPaddlePriceId || null,
    is_published,
    meta_title,
    meta_description,
    og_image,
 ...
 */
export async function replaceProductFilePath(
  productId: string,
  nextFilePath: string,
) {
  const supabase = await createClient();
  ...
  if (prev && prev !== nextFilePath) {
    ...
    if (res.error)
      console.warn("Storage cleanup failed (product file):", res.error);
  }
