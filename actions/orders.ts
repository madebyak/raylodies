...
type UserJoin = { email: string; full_name: string | null } | { email: string; full_name: string | null }[] | null
type ProductJoin = { title: string; thumbnail: string | null } | { title: string; thumbnail: string | null }[] | null
  ...
  return {
    ...
    order_items: (row.order_items || []).map((item) => {
      ...
      return {
        ...
        price: item.price,
        product: product ? { title: product.title, thumbnail: product.thumbnail } : undefined,
      }
    }),
  }
}
...
export async function getOrderStats() {
  ...
  const { data: statsRows, error: rpcError } = await supabase
    .rpc('get_order_stats')
