...
export async function addProjectMedia(
  ...
  console.log(
    `Adding project media: projectId=${projectId}, type=${type}, url=${url}`,
  );
...
export async function removeProjectMedia(id: string) {
  ...
  for (const candidate of [existing?.url, existing?.poster_url]) {
    ...
    if (res.error)
      console.warn("Storage cleanup failed (project media):", res.error);
  }
...
export async function getProjectMedia(projectId: string) {
  ...
  console.log(
    `Fetched ${data?.length || 0} media items for project ${projectId}`,
  );
