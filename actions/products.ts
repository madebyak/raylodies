...
type CategoryJoin =
  | { id: string; name: string; slug: string; type: "project" | "product" }
  | { id: string; name: string; slug: string; type: "project" | "product" }[]
  | null;
...
export const getPublishedProducts = cache(async () => {
  ...
  return (
    ...
    price: p.price,
    is_free:
      Boolean((p as unknown as { is_free?: boolean | null }).is_free) ||
      p.price === 0,
...
export async function deleteProduct(
  id: string,
): Promise<{ success?: true; archived?: true; error?: string }> {
  const supabase = await createClient();
